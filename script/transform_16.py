...
def flatten_data(data):
    def get_df(inp, record_path):
        df_sub = pd.json_normalize(inp, record_path=[record_path], meta=['Tahun', 'Bulan', 'Dati_II', 'Kurs'])
...
def Transform16(file_p, excel_file_p):
    ...
    # Reorder columns
    df_all = df_all[['Propinsi', 'Dati_II', 'Tahun', 'Bulan', 'Kurs', 'Tipe', 'Nominal']]
...
if __name__ == "__main__":
    ...
    for province in province_dirs:
        file_path = os.path.join(parent_dir, 'data', latest_dir, province, "ii16", "./ii16.xls") 
        excel_file_path = os.path.join(parent_dir, 'data', latest_dir, province, "ii16", "summary_16.csv")
